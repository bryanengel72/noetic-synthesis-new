...
| `insights/index.html` | the Insights landing page (`/insights/`): a horizontal rail of cover cards in series order (part 1 on the left). Clicking a card opens the essay in a full-screen reader loaded from the essay's own page (`?read=<slug>` deep-links to it; without JS the card is a plain link). Cover art is generated from `data-cover` (any integer), so no image files. Add a new essay as an `<a class="card">` here in series position, plus `sitemap.xml` and `llms.txt`. All six parts are in; there is no placeholder card. |
| `insights/<slug>.html` | one page per essay. Julie's series is six parts in a fixed order, part 1 `your-ai-isnt-failing-your-thinking-is.html` through part 6 `the-decision-ai-cannot-make-for-you.html`. Source PDFs came from Julie; the text was converted by hand. |
